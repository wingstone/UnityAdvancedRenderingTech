...
float3 GetMultipleScattering(sampler2D multiScatteringLut, AtmosphereParameters Atmosphere, float3 worlPos, float viewZenithCosAngle,float2 multiScatteringLutRes)
{
	...
	float3 multiScatteredLuminance = tex2D(multiScatteringLut, uv).rgb;
	return multiScatteredLuminance;
}

float3 GetMultipleScattering(Texture2D<float4> multiScatteringLut, AtmosphereParameters Atmosphere, float3 worlPos, float viewZenithCosAngle,float2 multiScatteringLutRes)
{
	float2 uv = saturate(float2(viewZenithCosAngle*0.5f + 0.5f, (length(worlPos) - Atmosphere.bottom_radius) / (Atmosphere.top_radius - Atmosphere.bottom_radius)));
	uv = float2(fromUnitToSubUvs(uv.x, multiScatteringLutRes.x), fromUnitToSubUvs(uv.y, multiScatteringLutRes.y));

    return multiScatteringLut.SampleLevel(samplerLinearClamp, uv, 0).rgb;
}
...
float3 RenderSkyViewLut(AtmosphereParameters atmosphere, sampler2D transmittanceLut, float cameraHight, float2 uv, float3 lightDirection, float4 groundColor, sampler2D multiScatteringLut, float4 multiScatteringLut_Size)
{
    ...
    { 
        ...
        float3 multiScatteredLuminance = GetMultipleScattering(multiScatteringLut, atmosphere, samplePosition, SunZenithCosAngle, multiScatteringLut_Size.zw);
...
#ifdef Use_GetSingleScattering_NoGround
...
void GetScattering_NoGround(AtmosphereParameters atmosphere, Texture2D<float4> transmittanceLut, float3 camerapos, float3 sunDirection, float3 eyeRay, float ray2voxelLength, Texture2D<float4> multiScatteringLut, float4 multiScatteringLut_Size,  out float3 outtransmittance, out float3 L)
{
    ...
    rayLength = min(rayLength, ray2voxelLength);
    float segmentLength = rayLength / SAMPLE_COUNT; 
    ...
    L = col;
    outtransmittance = GetTransmittance(atmosphere, transmittanceLut, r, mu, rayLength, rayHitEarth);
}
